...
const FilterComponent2: React.FC<{ setModalVisible: any }> = ({ setModalVisible }) => {
    //  selected filters
    ...
    const createFiterAtribute=()=>{
    const arrayofObject:any =[]
        
        availableFilters.forEach((element:any,index:any) => {
            if(index === 0){
                setSelected(element.displayName);
            }
        let   obj = { name: element.displayName, query: element.nameOrId}
        arrayofObject.push(obj)
        });
        setfilterAttributes(arrayofObject)
    }
    ...
    const AddFilter = (updatedData: any) => {
        ...
        if (updatedData?.length > 0) {
            const filteres = updatedData.flatMap((category: any) =>
                category.values.filter((product: any) => product.select)
            );
              setFilters(filteres);
        }
    }
    ...
    const isCheckFillter = () => {
        ...
        if (filterList?.length != 0) {
            ...
            const updatedMainArray = data.map((brand: any) => {
                const updatedProducts = brand.values.map((filter: any) => {
                ...
                return { ...brand, values: updatedProducts };
            });
    ...
    const handleCheckBox = (value: any, mainIndex: any, subIndex: any) => {
        ...
        const updatedData = CloneData.map((item, index) => {
            if (item && index === mainIndex) {
                const updatedChildren = item.values.map((child: any, subIn: any) => {
                    if (subIn === subIndex) {
                        return { ...child, select: value };
                    }
                    return child;
                });
                return { ...item, values: updatedChildren };
            }
            return item;
        });
        ...
        if (updatedData?.length > 0) {
          const filteredCategories = updatedData.map((category: any) => {
            if (category?.values && category.values?.length > 0) {
              return category.values.filter((filter: any) => {
                if (filter?.select) {
            ...
            <View style={styles.container}>
                ...
                <View style={{ flex: 0, flexDirection: "row" }}>
                    ...
                    <View style={[styles.leftBox]} >
                        {
                           filterAttributes.length >0 && filterAttributes.map((item:any, i) => {
                    ...
                    <ScrollView style={[styles.rightBox]} >
                        {
                            data && data.length > 0 && data.map((Item: any, index: any) => {
                                ...
                                return (
                                    <View key={index}>
                                        {
                                            Item?.values?.length > 0 && Item.values.map((element: any, i: any) => {
                                                if (selected === Item.displayName) {
                                                    return (
                                                        <View key={i} style={{ alignItems: "center", flexDirection: "row", marginLeft: 10 }}>
                                                            ...
                                                            <Text style={styles.brandName}>{element?.displayName} ({element?.productCount})</Text>
                                                        </View>
                                                    );
                                                }
                                            })
